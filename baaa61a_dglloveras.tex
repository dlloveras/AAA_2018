...
While the electron density determined by WL tomography is of an absolute nature, that obtained from EUV tomography is dependent on the assumed iron coronal abundance. Also, while EUV tomography is affected by the so-called coronal flling factor, the WL tomographic results are not. No comparison has been yet carried-out between results obtained with both techniques. In this work the tomographic reconstruction of the coronal electron density for a given rotation is carried out for the first time using both methods and results are compared. 
